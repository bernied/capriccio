...
/*
    ...
    struct coroutine *cothread_new(void (*func)(), ...)
    ...
    void *cothread_schedule(void)
	Give up processing and let other coroutines run.  To restart
	this one, another coroutine has to co_call back.  It returns
	the data pointer the other coroutine passed to co_call.
...
struct ioreq {
    struct ioreq *next;
    struct coroutine *coro;	// coroutine that is waiting
...
///// first some timeval arithmetic routines /////

static struct timeval *
tvadd(struct timeval *dst, struct timeval *a, struct timeval *b)
{
...
static struct timeval *
tvsub(struct timeval *dst, struct timeval *a, struct timeval *b)
{
    dst->tv_sec = a->tv_sec - b->tv_sec;
    dst->tv_usec = a->tv_usec - b->tv_usec;
    if (dst->tv_usec < 0)
...
static struct timeval *
to2tv(struct timeval *dst, int timeout)
{
...
static void
set_fds(struct ioreq *r, int mode, fd_set *fds, fd_set **fp)
{
    if (r->mode & mode) {
	FD_SET(r->fd, fds);
	*fp = fds;
...
static int
tst_fds(struct ioreq *r, int mode, fd_set *fds)
{
    if (r->mode & mode)
...
static int
check(struct ioqueue *q, struct ioreq *r, struct timeval *ctime)
{
    int res = 0;
...
static void
enqueue(struct ioqueue *q, struct ioreq *r)
{
    if (r->mode & (IOREAD|IOWRITE|IOEXCEPT)) {
...
static void
vadd_req(struct ioreq *r, int mode, va_list args)
{
    r->coro = co_current;
    r->mode = mode;
...
static void
add_req(struct ioreq *r, int mode, ...)
{
    va_list args;
    va_start(args, mode);
...
cothread_wait(int mode, ...)
{
    ...
    va_end(args);
    return *(int*)cothread_schedule();

}

struct coroutine *
cothread_new(void (*func)(), ...)
{
    struct coroutine *co;

    if ((co = co_create(func, 0, 32768))) {
	va_list args;
	struct ioreq req[1];
...
// a ticker: prints every second a message.
static void
test1(va_list args)
{
...
// a copyer: copy data from one fd to another
static void
test2(va_list args)
{
    char *str = va_arg(args, char *);
